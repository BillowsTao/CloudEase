...
@WebServlet(name = "FileUplodaServlet", urlPatterns = "/page/upload.do")
public class FileUplodaServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        uploadPath = FileManage.getTwitterImgPath(getServletContext().getRealPath("./"));
        // 如果目录不存在则创建
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    ...
                    if (!item.isFormField()) {
                        ...
                        System.out.println(File.separator + FileManage.twitterImageUrl + File.separator + fileName);
                        System.out.println("***************************************************************************");
                        response.getWriter().write(File.separator + FileManage.twitterImageUrl + File.separator + fileName);
                    }
                }
            }
        } catch (Exception ex) {
